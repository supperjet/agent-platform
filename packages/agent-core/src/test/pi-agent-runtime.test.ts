...
test("prints playground run and event store records", () => {
  const cli = createCliInvocation();
  const root = mkdtempSync(join(tmpdir(), "agent-core-playground-output-"));
  try {
    const result = spawnSync(cli.command, [
      ...cli.args,
      "--faux",
    ...
    assert.equal(result.status, 0, result.stderr);
    assert.match(result.stdout, /playground answer/);
    assert.match(result.stdout, /"runId": "playground-run-1"/);
    assert.match(result.stdout, /"commandId": "playground-command-1"/);
    assert.match(result.stdout, /"status": "succeeded"/);
    assert.match(result.stdout, /"type": "run_started"/);
    assert.match(result.stdout, /"type": "run_finished"/);
    assert.equal(result.stdout.includes('"runId": "playground-run-2"'), false);
  } finally {
