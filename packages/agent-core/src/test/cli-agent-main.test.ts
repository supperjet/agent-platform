...
test("formatSkill includes support trust policy diagnostics", () => {
  ...
  assert.equal(formatSkill(registry, "collect"), [
    ...
    "  trust: read=no, inject=no, execute=no",
    "  reason: scripts require SkillRuntime and ToolRuntime approval before they can be read or executed",
    "",
    "Collect local data.",
    "",
    "support file diagnostics:",
    "- warning: trust-policy-denied: Skill support file read denied by trust policy: scripts require SkillRuntime and ToolRuntime approval before they can be read or executed",
    "  path: /agent/skills/collect/scripts/collect.ts",
    "",
  ].join("\n"));
});
